...
| 序号 | 文档 | 内容说明 |
|------|------|---------|
| 16 | [本地编译部署与测试操作指南](16-本地编译部署与测试操作指南.md) | 各模块本地编译、启动、联调、测试的具体操作步骤 |
| 17 | [设备启动流程运行验证报告](17-设备启动流程运行验证报告.md) | **Docker 容器端到端运行验证**：OTA→WebSocket→绑定校验→语音对话→绑定流程 3 场景（Mock manager-api），含 ESP32 固件侧源码交叉对照，14 项验证全部通过 |
