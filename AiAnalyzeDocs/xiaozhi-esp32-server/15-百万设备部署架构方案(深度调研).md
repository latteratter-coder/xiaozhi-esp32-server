...
manager-api 共 **12 个业务模块**，理解各模块职责才能精准定位扩展瓶颈：
...
| **device** | 设备注册(6位码)、绑定/解绑、在线状态、OTA版本检查、工具调用转发、通讯录管理 | `/device/register`、`/device/bind`、`/ota/`、`/otaMag/upload` | 设备连接与OTA |
| **config** | 为 Python 服务端组装运行配置，按 MAC 解析已选模型、下发替换词 | `/config/server-base`、`/config/agent-models`、`/config/correct-words` | 频繁配置读取 |
...
| **correctword** | ASR 替换词文件管理、智能体关联、下发给 Python 服务 | `/correct-word/file/*`、`/config/correct-words` | 无明显瓶颈 |
